...
from transformers.models.bert.modeling_bert import BertAttention, BertLayer, BertEncoder, BertModel
...
class MyBertSelfAttention(nn.Module):
    def __init__(self,
        ...
        # LSY) Use normal softmax or Clipped softmax
        self.softmax_fn = softmax_fn

        # LSY) Create Gated layer
        # Referred: https://github.com/Qualcomm-AI-research/outlier-free-transformers/blob/main/transformers_language/models/bert_attention.py#L16
        self.use_mlp_gating = use_mlp_gating
        self.gate_list = None
        if self.use_mlp_gating:
            module_list = []
            # LSY) Reffered from writer's git, but suspicous.
            ...
            self.gate_list = nn.ModuleList(module_list)
    ...
    def forward(
        ...
        # LSY) Normalize the attention scores to probabilities with defined softmax function
        ...
        if self.use_mlp_gating:
            ...
            for head_idx in range(self.num_attention_heads):
                x_head = x[:, head_idx, ...]
                fc_head = self.gate_list[head_idx]
            ...
            context_layer *= gate # element-wise product
