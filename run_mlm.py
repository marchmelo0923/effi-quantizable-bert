...
from quantizable_bert.create_bert import create_QuantizableBert
...
def main():
    ...
    # LSY) Set multi gpu timeout value to 2 hours. (Before 30 minutes, it always stucked)
    ...
    ###################################################
    # LSY) move 'model creating' to create_bert.py
    ###################################################
    ...
    model = create_QuantizableBert(config=config,
                                   model_cache_dir=args.model_cache_dir,
                                   softmax_type=args.attn_softmax,
                                   alpha=args.alpha,
                                   max_seq_length=args.max_seq_length,
                                   gamma=args.gamma,
                                   zeta=args.zeta,
                                   use_mlp_gating=args.use_mlp_gating,
    ...
    if dataset_setup == DatasetSetups.bookcorpus_and_wiki and tokenized_book_wiki_path.exists():
        ...
        if dataset_setup == DatasetSetups.wikitext_2:
            ...
            raw_datasets["train"] = load_dataset(
                "wikitext", "wikitext-2-raw-v1", cache_dir=args.data_cache_dir, split=train_split, trust_remote_code=True
            )
            raw_datasets["validation"] = load_dataset(
                "wikitext", "wikitext-2-raw-v1", cache_dir=args.data_cache_dir, split=val_split, trust_remote_code=True
            )

        elif dataset_setup == DatasetSetups.wikitext_103:
            raw_datasets = DatasetDict()
            raw_datasets["train"] = load_dataset(
                "wikitext", "wikitext-103-raw-v1", cache_dir=args.data_cache_dir, split=train_split, trust_remote_code=True
            )
            raw_datasets["validation"] = load_dataset(
                "wikitext", "wikitext-103-raw-v1", cache_dir=args.data_cache_dir, split=val_split, trust_remote_code=True
            )

        elif dataset_setup == DatasetSetups.bookcorpus_and_wiki:
            bookcorpus = load_dataset(
                "bookcorpus", cache_dir=args.data_cache_dir, split=train_split, trust_remote_code=True
            )

            wiki_train = load_dataset(
                "wiki40b", "en", cache_dir=args.data_cache_dir, split=train_split, trust_remote_code=True
            )
            wiki_val = load_dataset("wiki40b", "en", cache_dir=args.data_cache_dir, split=val_split, trust_remote_code=True)
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(args.log_name, experiment_config, init_kwargs={"wandb": {"entity": args.wandb_entity}})
    ...
    if hasattr(model, 'module'):  # DDP 모델인 경우
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.with_tracking and completed_steps % args.tb_scalar_log_interval == 0:
                ...
                for layer_idx in range(len(base_model.bert.encoder.layer)):
                    self_attn_layer = base_model.bert.encoder.layer[layer_idx].attention.self
